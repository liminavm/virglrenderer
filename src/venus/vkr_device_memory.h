...
struct vkr_device_memory {
   ...
   /* gkvm tier-2 (macOS): borrowed ref to the global IOSurface backing the VkImage this
   ...
   /* gkvm tier-2 (macOS): RETAINED IOSurfaceRef of another context's window buffer this
   ...
   /* gkvm snapshot-replay P2: this memory was created by importing a virgl
...
/* gkvm snapshot-replay P2: whether this memory's bytes must be captured by the
...
/* gkvm snapshot-replay P2: copy min(size, allocation_size) bytes between `buf`
