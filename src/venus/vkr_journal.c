...
/* sized from the generated dispatch table itself so a protocol uprev can never
 * silently strand new commands outside the class table (a hardcoded 346 would
 * have missed the command maintenance 8-11 added — new vkCmd* entries would
 * quietly lose their RECORDING class and snapshot replay would skip them) */
#define VKR_JOURNAL_NUM_CMD_TYPES ARRAY_SIZE(vn_dispatch_table)
...
/* limina (2026-07-28, decode-lane block batching): while a decode thread drains
