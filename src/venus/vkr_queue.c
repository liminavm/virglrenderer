...
{
   TRACE_FUNC();
   /* gkvm: a present-fence sync retires the shared present fence (refcounted
    * across the context's queues), not a guest fence. */
   if (sync->gkvm_present) {
      vkr_present_fence_release(sync->gkvm_present);
...
vkr_queue_sync_submit_present(struct vkr_queue *queue, struct vkr_present_fence *pf)
{
   ...
   struct vkr_queue_sync *sync = vkr_device_alloc_queue_sync(
      dev, VIRGL_RENDERER_FENCE_FLAG_MERGEABLE, VKR_GKVM_PRESENT_RING, pf->fence_id);
   if (!sync)
      return false;
   sync->gkvm_present = pf;
   ...
   if (result == VK_ERROR_DEVICE_LOST) {
      ...
      vkr_log("gkvm present sync submit failed (vk ret %d)", result);
      return false;
   }
...
{
   /* gkvm P2.1: capture the vkr_device before create_and_add replaces the
    * handles in args. */
   struct vkr_device *dev = vkr_device_from_handle(args->device);
...
{
   /* gkvm P2.1: capture device + timeline-ness before handles are replaced. */
   struct vkr_device *dev = vkr_device_from_handle(args->device);
   ...
   if (obj) {
      obj->device = dev;
      obj->gkvm_is_timeline = is_timeline;
   }
}
