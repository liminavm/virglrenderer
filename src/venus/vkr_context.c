...
#ifdef __APPLE__
   /* gkvm #28: HOST_VISIBLE blob shares MoltenVK's own vkMapMemory pointer (blob.map_ptr) — no
...
vkr_context_ring_monitor_thread(void *arg)
{
   ...
   /* gkvm: stamp at 1/3 of the requested MAX reporting period (an upper
   ...
   while (ctx->ring_monitor.started) {
      if (ret == thrd_busy) {
         /* gkvm: a stamp gap beyond the REQUESTED period means the guest
...
#ifdef __APPLE__
         /* gkvm: fd-pressure canary. The worker is a long-lived singleton and a
...
vkr_context_destroy(struct vkr_context *ctx)
{
   ...
   /* gkvm journal: detach before the teardown sweeps below so their object
                   ...
                   const char *debug_name)
{
   ...
   /* gkvm: snapshot-replay journal; best-effort (NULL disables recording) */
