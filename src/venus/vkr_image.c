...
#define GKVM_MTLPixelFormatRGBA8Unorm_sRGB 71
#define GKVM_MTLPixelFormatBGRA8Unorm 80
#define GKVM_MTLPixelFormatBGRA8Unorm_sRGB 81

/* EXACT MTLPixelFormat for a VkFormat — sRGB stays sRGB.
 *
 * This is deliberately NOT what gkvm_vkformat_to_iosurface() returns. That one folds sRGB onto
...
gkvm_vkformat_to_mtl_exact(VkFormat fmt, uint32_t *mtl)
{
   ...
   case VK_FORMAT_B8G8R8A8_SRGB:
      *mtl = GKVM_MTLPixelFormatBGRA8Unorm_sRGB;
      return true;
   case VK_FORMAT_R8G8B8A8_UNORM:
      *mtl = GKVM_MTLPixelFormatRGBA8Unorm;
      return true;
   case VK_FORMAT_R8G8B8A8_SRGB:
      *mtl = GKVM_MTLPixelFormatRGBA8Unorm_sRGB;
      return true;
   default:
      return false;
   }
                           ...
                           struct vn_command_vkCreateImage *args)
{
   ...
   if (ci && ci->pNext) {
      ...
      if (ext && ext->handleTypes) {
         ...
         struct vkr_device *gkvm_early_dev = vkr_device_from_handle(args->device);
         const bool gkvm_native_mod =
            gkvm_early_dev && gkvm_early_dev->physical_device &&
            !gkvm_early_dev->physical_device->EXT_metal_objects &&
            gkvm_early_dev->physical_device->EXT_image_drm_format_modifier &&
         ...
         for (VkBaseInStructure *s = (VkBaseInStructure *)mci->pNext; s;
            ...
            if (t == VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO ||
                (is_mod && !gkvm_native_mod))
               prev->pNext = s->pNext; /* unlink */
            else
               prev = s;
         }
         if (mci->tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT && !gkvm_native_mod)
         ...
         if (gkvm_vkformat_to_iosurface(ci->format, &gkvm_mtl, &gkvm_fourcc, &gkvm_bpe)) {
            ...
            if (gkvm_is_import) {
               ...
               if (!dev->physical_device->EXT_metal_objects && !gkvm_native_mod) {
               ...
               if (gkvm_mtltex) {
                  ...
                  uint32_t gkvm_mtl_exact = gkvm_mtl;
                  gkvm_vkformat_to_mtl_exact(ci->format, &gkvm_mtl_exact);
                  gkvm_surf = vkr_mtl_iosurface_alloc(dev->mtl_device, ci->extent.width,
                                                      ci->extent.height, gkvm_mtl_exact,
                                                      gkvm_fourcc, gkvm_bpe, 0);
                  fprintf(stderr,
                          "[LIMINA-VKR-MTLTEX] KK MTLTEXTURE scanout %ux%u -> IOSurface "
                          "id=%u tex=%p\n",
                          ci->extent.width, ci->extent.height,
                          gkvm_surf ? gkvm_surf->id : 0,
                          gkvm_surf ? gkvm_surf->mtl_texture : NULL);
                  vkr_log("gkvm: KK MTLTEXTURE scanout %ux%u -> IOSurface id=%u tex=%p",
                          ci->extent.width, ci->extent.height,
                          gkvm_surf ? gkvm_surf->id : 0,
                          gkvm_surf ? gkvm_surf->mtl_texture : NULL);
   ...
   if (gkvm_obj && ci)
      gkvm_obj->gkvm_vk_format = (uint32_t)ci->format;
