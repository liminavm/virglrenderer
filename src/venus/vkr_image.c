                           ...
                           struct vn_command_vkCreateImage *args)
{
   ...
   if (ci && ci->pNext) {
      ...
      if (ext && ext->handleTypes) {
         ...
         if (!(gkvm_ht_seen & ext->handleTypes)) {
            gkvm_ht_seen |= ext->handleTypes;
         ...
         static int gkvm_keep_structs = -1;
         if (gkvm_keep_structs < 0) {
            const char *e = getenv("LIMINA_VKR_KEEP_MODIFIER_STRUCTS");
            gkvm_keep_structs = (e && *e == '1') ? 1 : 0;
         }
         ...
         if (mci->tiling == VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT) {
            ...
            static unsigned gkvm_mod_hits;
            if (++gkvm_mod_hits <= 4 || (gkvm_mod_hits % 256) == 0)
               fprintf(stderr, "[LIMINA-VKRMOD] modifier-tiled guest image #%u %s import=%d\n",
                       gkvm_mod_hits, gkvm_is_import ? "EXPLICIT" : "LIST", gkvm_is_import);
         ...
         if (gkvm_vkformat_to_iosurface(ci->format, &gkvm_mtl, &gkvm_fourcc, &gkvm_bpe)) {
            ...
            if (gkvm_is_import) {
               ...
               static int gkvm_no_force = -1;
               if (gkvm_no_force < 0) {
                  const char *e = getenv("LIMINA_VKR_NO_KK_FORCE_LINEAR");
                  gkvm_no_force = (e && *e == '1') ? 1 : 0;
               }
               ...
               if ((!gkvm_mtltex || !gkvm_surf) && !gkvm_no_force) {
                  mci->tiling = VK_IMAGE_TILING_LINEAR;
