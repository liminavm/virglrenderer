...
/* gkvm: VKR_FD_TRACE=1 turns on per-event fd tracing (mtl_shm carrier
...
/* gkvm KK bring-up command tracing (GKVM_KK_RTLOG). Cached: the raw getenv in
...
vkr_kk_rtlog(void)
{
   static int on = -1;
   if (on < 0) {
      const char *e = getenv("GKVM_KK_RTLOG");
      on = e && e[0] && strcmp(e, "0") != 0;
   }
   return on;
}
...
/* gkvm fence-accurate present (#8/#31): the reserved per-context fence ring the
...
#define VKR_GKVM_PRESENT_RING 63
