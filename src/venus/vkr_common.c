...
static const struct vn_info_extension_table vkr_extension_table = {
   ...
   /* gkvm (macOS only): DISABLED — MoltenVK's uint8->uint16 index-conversion compute
...
#ifdef __APPLE__
   .EXT_index_type_uint8 = false, /* gkvm: see KHR_index_type_uint8 above (MoltenVK uint8 conv bug) */
#else
