                       ...
                       struct virgl_context_blob *blob)
{
   ...
   /* limina tier-2 (macOS) #28: a HOST_VISIBLE blob is shared by pointer (MoltenVK's own
   ...
   blob->u.fd = reply_fd;
   blob->map_info = reply.map_info;
   /* limina tier-2 (macOS): carry the scanout IOSurface id reconstructed from the render-server
...
proxy_context_attach_resource(struct virgl_context *base, struct virgl_resource *res)
{
   ...
   /* limina: a CLASSIC vrend pipe resource whose texture is IOSurface-backed (a
